...
def run():
    with open(f"../data/totals_playoffs/{season_input}_player_totals_playoffs.csv", 'r') as file:
        ...
        for row in reader:
            ...
            try:
                player = PlayerDataTotalsPlayoffs.objects.filter(
                    player_id=player_id,
                    season=season_int,
                    team=team
                ).first()
                ...
                if player:
                    ...
                    new_player = PlayerDataTotalsPlayoffs.objects.create(
                        player_id=player_id,
                        season=season_int,
                        team=team,
                        ...
                        games_started=row['GS'] if row['GS'] else None,
                        minutes_pg=row['MP'] if row['MP'] else None,
                        field_goals=row['FG'] if row['FG'] else None,
                        field_attempts=row['FGA'] if row['FGA'] else None,
                        field_percent=row['FG%'] if row['FG%'] else None,
                        three_fg=row['3P'] if row['3P'] else None,
                        three_attempts=row['3PA'] if row['3PA'] else None,
                        three_percent=row['3P%'] if row['3P%'] else None,
                        two_fg=row['2P'] if row['2P'] else None,
                        two_attempts=row['2PA'] if row['2PA'] else None,
                        two_percent=row['2P%'] if row['2P%'] else None,
                        effect_fg_percent=row['eFG%'] if row['eFG%'] else None,
                        ft=row['FT'] if row['FT'] else None,
                        ft_attempts=row['FTA'] if row['FTA'] else None,
                        ft_percent=row['FT%'] if row['FT%'] else None,
                        offensive_rb=row['ORB'] if row['ORB'] else None,
                        defensive_rb=row['DRB'] if row['DRB'] else None,
                        total_rb=row['TRB'] if row['TRB'] else None,
                        assists=row['AST'] if row['AST'] else None,
                        steals=row['STL'] if row['STL'] else None,
                        blocks=row['BLK'] if row['BLK'] else None,
                        turnovers=row['TOV'] if row['TOV'] else None,
                        personal_fouls=row['PF'] if row['PF'] else None,
                        points=row['PTS'] if row['PTS'] else None,
                    )
                    print(f"Created new record for player: {new_player.player_name}")
